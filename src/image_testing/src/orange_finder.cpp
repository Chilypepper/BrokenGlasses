    ...
    for(double cols = cv_ptr->image.cols; pointx < cols; pointx+=cols/accuracy){
      ...
      for(double rows = cv_ptr->image.rows; pointy < rows; pointy+=rows/accuracy){
        if(cv_ptr->image.at<Vec3b>(Point(pointx,pointy))[0] > 0  &&
           cv_ptr->image.at<Vec3b>(Point(pointx,pointy))[0] < 25 &&
           cv_ptr->image.at<Vec3b>(Point(pointx,pointy))[1] > 35  &&
           cv_ptr->image.at<Vec3b>(Point(pointx,pointy))[1] < 50 &&
           cv_ptr->image.at<Vec3b>(Point(pointx,pointy))[2] < 125  &&
           cv_ptr->image.at<Vec3b>(Point(pointx,pointy))[2] > 110){
            #ifdef findPoint
